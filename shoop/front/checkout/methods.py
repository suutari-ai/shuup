...
class MethodModelChoiceField(forms.ModelChoiceField):
    ...
    def label_from_instance(self, obj):
        ...
        """
        label = force_text(obj.get_effective_name(self.cart))

        price_info = (
            obj.get_total_cost(self.cart)
            if self.cart and self.show_prices else None)
...
class MethodsForm(forms.Form):
    ...
    def limit_method_fields(self):
        cart = self.cart  # type: shoop.front.cart.objects.Cart
